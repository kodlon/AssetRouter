...
{
    /// <summary>
    /// Instantiates a template prefab, calls
    /// <see cref="IAssetRouterPrefabSetup.SetupAssetRouter" /> on all
    ...
    {
        /// <summary>
        /// Prefab to use as the template. The instance is created, configured,
        /// and saved.
        /// </summary>
        public GameObject templatePrefab;

        /// <summary>
        /// Folder where the output prefab is saved. Falls back to the imported
        /// asset's folder when empty.
        /// </summary>
        [Tooltip("Output folder. Empty = the imported asset's folder.")]
        public string outputFolder = "";

        /// <summary>
        /// File name for the output prefab without extension. <c>{assetName}</c> is
        /// replaced with the imported file
        /// name (without extension). Example: <c>{assetName}_Prefab</c> on
        /// <c>Character.fbx</c> produces <c>Character_Prefab.prefab</c>.
        /// </summary>
        [Tooltip("{assetName} is replaced with the imported file name (no extension).")]
        public string namePattern = "{assetName}_Prefab";

        /// <summary>
        /// When false, the action is skipped if a prefab already exists at the
        /// output path.
        /// </summary>
        public bool overwriteExisting;

        public override bool CanRunOn(Object importedAsset, AssetImportContext ctx) => templatePrefab != null;
