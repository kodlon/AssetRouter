...
namespace Kodlon.AssetRouter.Actions
{
    ...
    /// Runs only on Texture2D assets. When <see cref="baseMaterial"/> is null, falls back to the active render pipeline's
    /// default material (URP/HDRP <c>RenderPipelineAsset.defaultMaterial</c>) or the Built-in <c>Default-Material</c>.
    /// </summary>
    ...
    {
        /// <summary>
        /// Material to use as the base. The new material copies its shader and all property values.
        /// When null, the action falls back to the active render pipeline's default material — useful for zero-config setups.
        /// </summary>
        public Material baseMaterial;

        /// <summary>
        /// Shader property to assign the texture to. Examples: <c>_MainTex</c> (Standard), <c>_BaseMap</c> (URP Lit).
        /// If the property does not exist in the shader, the action falls back to <c>Material.mainTexture</c>, which
        /// respects the shader's <c>[MainTexture]</c> attribute (URP Lit maps this to <c>_BaseMap</c>).
        /// </summary>
        [Tooltip("Shader property to assign the texture to. Falls back to Material.mainTexture if not present on the shader.")]
        public string textureProperty = "_MainTex";

        /// <summary>
        /// Output folder for the .mat. Absolute paths starting with <c>Assets/</c> are used as-is; a relative path
        /// (e.g. <c>Materials</c>) is resolved as a subfolder of the imported texture's folder. Empty falls back to
        ...
        public override bool CanRunOn(Object importedAsset, AssetImportContext ctx)
            => importedAsset is Texture2D && ResolveBaseMaterial() != null;
        ...
        /// <summary>
        /// Explicit <see cref="baseMaterial"/> → active SRP <c>defaultMaterial</c> (URP/HDRP) → Built-in RP <c>Default-Material</c>.
        /// </summary>
