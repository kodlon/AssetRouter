...
### 12.12 Beginner guide + UI reference (додано 2026-07)
...
- [ ] `Documentation~/getting-started.md` — покрокова інструкція з нуля: встановлення → що створилось автоматично → перший drag-and-drop → створення власного першого правила → де дивитись, чому файл не поїхав (Diagnostic Window). Проста мова, без жаргону, скріншот на кожен крок. Мета: користувач без досвіду editor-тулзів досягає першого "вау" за 10 хв.
- [ ] `Documentation~/ui-reference.md` — повний UI-довідник, **структурований по вікнах, не суцільним списком**. Ієрархія документа:
  - Розділ на кожне вікно: головне вікно Asset Router (і в ньому підрозділ на кожну з 4 вкладок — Settings / Dry Run / History / Validate), Diagnostic Window, меню `Tools/Asset Router/...`, контекстне меню `Assets/Create/Asset Router/...` (wizard).
  - Кожен розділ починається з 2-3 речень: **що це вікно/вкладка робить і навіщо існує** ("Dry Run — безпечний перегляд: показує, що станеться з файлами, до того як щось рухати").
  - Далі елементи **в порядку, як вони йдуть на екрані** (toolbar зліва направо → зверху вниз), не в рандомному: таблиця "Елемент → Що робить → Коли потрібен", 1-2 речення на елемент.
  - Не забути неочевидне: toolbar головного вікна (Database picker, Create New, Export/Import JSON), кнопки в деталях правила (Copy/Paste Rule, списки actions), banner конфліктів, multi-database warning.
- [ ] Лінки на обидва файли з `index.md`, `README.md` і `DOCUMENTATION_EN.md` (getting-started — першим пунктом).

### 12.14 Feature catalog — повний перелік фіч з прикладами (додано 2026-07)

Фіч уже стільки, що навіть автор забуває, що вміє плагін. Потрібен один документ-каталог — і для користувачів, і як основа для презентацій/сторінки в сторі:

- [ ] `Documentation~/features.md` — **кожна** фіча плагіна: 1 абзац опису (що робить і навіщо) + 1 конкретний приклад застосування (сценарій "було → стало", в ідеалі з реального пайплайна).
- [ ] Покрити щонайменше: auto-routing по правилах, glob/regex патерни, path templating (`{1}`/`{name}`), preset auto-apply, post-import actions (+ список 10 built-in одним рядком кожен, лінк на існуючі per-action доки), scope folder, dry run, batch re-import (force in-place), history + undo, conflict detection, unknown files dialog, diagnostic window, per-rule statistics, naming validator (Validate tab), JSON export/import бази, rule sharing через clipboard, action scaffolding wizard, multi-database warning.
- [ ] Групувати по призначенню (Routing / Safety / Extensibility / Team workflow), не по порядку реалізації.
- [ ] Лінк з `README.md` і `index.md`. Цей же документ — джерело тез для презентації тех ліду та опису на сторі.
...
- [ ] Додати в `DefaultDatabaseFactory` хоча б одне правило з демо-action. Критерій вибору action: safe-by-default (не мутує існуючі ассети несподівано), ефект видимий одразу, без зовнішніх залежностей. Кандидати: `SetPivotAction` (0.5, 0.5) на UI Textures (безпечний, але ефект тихий) або `GenerateNineSliceBordersAction` на UI (ефект видно в Sprite Editor). Рішення за розробником.
- [ ] Action-ассет має створюватись фабрикою як sub-asset нової бази (як це робить UI при `+`), не лежати окремим файлом у пакеті.
- [ ] Оновити `DefaultDatabaseFactoryTests` (+1 тест: правило з action існує і action не null).
- [ ] Згадати в `getting-started.md` (12.12): "подивись на правило X — так підключаються actions".
