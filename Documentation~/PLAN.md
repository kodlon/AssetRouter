...
- [ ] **Perf (Medium):** `ConflictDetector.Detect` викликає `AssetDatabase.FindAssets` синхронно — тригериться через `ApplyModifiedProperties()` в `OnGUI`. При великих проєктах викликає лаг UI. Виправлення: кешувати результат, інвалідувати по `AssetDatabase.importPackageCompleted` / `projectChanged`.

### 11.15 AssetRouterInitializer — subscription accumulation
- [ ] **Low:** `EditorApplication.delayCall += Initialize` і `EditorApplication.projectChanged += RunMigratorIfNeeded` накопичують підписки при вимкненому Domain Reload (кожен `[InitializeOnLoadMethod]` додає ще один хендлер). Функціонально нешкідливо (обидва ідемпотентні), але треба додати unsubscribe-first: `event -= handler; event += handler;`.
...
- [ ] **v0.9.1** — Epic 16 (Path Templating: capture group tokens у targetFolder + 2 нові default rules + документація).
